...
class Custom_Search_Public {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/custom-search-public.css', array(), '', 'all' );

		$url_path = trim(parse_url(add_query_arg(array()), PHP_URL_PATH), '/');

		$url_path = explode('/', $url_path);
		 
		  if (isset($url_path[0]) && isset($url_path[1]) && $url_path[0] == 's' ) {
	...
	public function cs_template_redirect(){
		...
		$url_path = explode('/', $url_path);
		 ...
		 /*print_r($url_path);
		exit;*/
		//print_r($url_path);exit;
		  if (isset($url_path[0]) && isset($url_path[1]) && $url_path[1] == 's' ) {
		    ...
		    $count = $this->getCount(urldecode($url_path[1]));
	...
	function cs_pre_get_document_title($title){ 
		...
		$url_path = explode('/', $url_path);
		  ...
		  if (isset($url_path[0]) && isset($url_path[1]) && $url_path[0] == 's' ) {
	 	      return $this->getTitle(urldecode($url_path[1])).' | '.get_bloginfo('name');
		  }
	...
	function getCount($keyword){
		global $wpdb;
		$table = $wpdb->prefix.'search_forms';
	    $sql = "SELECT count FROM $table WHERE keyword='$keyword' LIMIT 1";
    	$results = $wpdb->get_results($sql,'ARRAY_A');
    	//print_r($results);
    	if($wpdb->num_rows > 0){
